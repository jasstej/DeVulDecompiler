...
start_parser.add_argument('--domain', default="devul.localhost", help='Domain name of host')
...
def start_server(args):
    ...
    cmd = f"docker stack deploy {config_files} --with-registry-auth --prune devuls-decompiler"

    subprocess.run(cmd.split(' '), env=env, check=True)


def stop_server():
    cmd = f"docker stack rm devuls-decompiler"
